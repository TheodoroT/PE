...
async function loadStateFromFirestore(uid) {
    try {
        ...
        if (doc.exists && doc.data().state) {
            // Firestore ALWAYS wins — overwrite local cache
            _applyRawState(doc.data().state);
            localStorage.setItem('pe_v6', JSON.stringify(state));
        } else {
            // Brand new user — start fresh, no local data migration
            state = {
        ...
        // Offline fallback — use local cache if available
        const raw = localStorage.getItem('pe_v6');
        if (raw) {
            try { _applyRawState(JSON.parse(raw)); } catch(e2) {}
        }
    }
}
...
function setupPhoneAuth() {
    ...
    document.getElementById('phone-back-btn')?.addEventListener('click', () => {
        ...
        if (document.getElementById('login-phone')) document.getElementById('login-phone').disabled = false;
    ...
    document.getElementById('phone-submit-btn')?.addEventListener('click', async () => {
        ...
        if (!_phoneConfirmation) {
            ...
            if (!phone) return showLoginError('Digite seu número de celular com DDD e código do país. Ex: +55 11 99999-9999');
            try {
                ...
                document.getElementById('phone-submit-btn').textContent = 'Verificar código';
                document.getElementById('login-phone').disabled = true;
                showLoginError('Código enviado! Verifique seu celular.');
                document.getElementById('login-error').style.color = 'var(--green)';
                document.getElementById('login-phone-code')?.focus();
            } catch(e) {
                _recaptchaVerifier = null; // reset on error
            ...
            try {
                ...
                showLoginError('Código inválido ou expirado. Tente novamente.');
            }
        }
    });
