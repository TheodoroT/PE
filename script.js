...
function setupEventListeners() {
    ...
    document.getElementById('dashboard-back-hoje')?.addEventListener('click', ()=>switchSection('hoje'));
    document.getElementById('metas-sort-select')?.addEventListener('change', ()=>renderMetas());
    document.getElementById('dashboard-back-hoje')?.addEventListener('click', ()=>switchSection('hoje'));
...
function moveTask(id, dir) {
    const k = getTaskPillar(id); if (!k) return;
    ...
    const sel = document.getElementById('metas-sort-select');
...
function renderMetas() {
    ...
    if (_metasView === 'gantt') {
        ...
        setTimeout(positionGanttTodayBar, 50);
        return;
    }
    ...
    const sortBy = document.getElementById('metas-sort-select')?.value || 'priority';
    const pw={high:3,medium:2,low:1};
    const statusOrder={'off-track':0,'at-risk':1,'on-track':2,'adiantada':3,'neutral':4};
    let sorted=[...allTasks].sort((a,b)=>{
        // Always put finished tasks at the bottom
        ...
        if(sortBy==='status') return (statusOrder[calcStatus(a).css]??9)-(statusOrder[calcStatus(b).css]??9);
        if(sortBy==='name') return a.text.localeCompare(b.text,'pt-BR');
        return 0;
    });
        ...
        <div class="task-header">
            ...
            <div class="task-menu-wrap">
                ...
                <div class="task-menu-dropdown">
                    <button class="task-menu-item" data-action="move-up" data-id="${t.id}">
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="18 15 12 9 6 15"/></svg>
                        Mover para cima
                    </button>
                    <button class="task-menu-item" data-action="move-down" data-id="${t.id}">
...
// ============================================================
function positionGanttTodayBar() {
    const bar = document.querySelector('.mg-today-positioned'); if (!bar) return;
    const nameEl = document.querySelector('.mg-name');
    const rows = bar.parentElement; if (!rows) return;
    const nameW = nameEl ? nameEl.getBoundingClientRect().width : 220;
    const rowsW = rows.getBoundingClientRect().width;
    const barArea = Math.max(1, rowsW - nameW);
    const pct = parseFloat(bar.dataset.pct) / 100;
    bar.style.left = (nameW + barArea * pct) + 'px';
...
function renderMetasGanttView(tasks, pillarKey) {
    ...
    return `<div class="mg-container">
        <div class="mg-timeline-header">
            ...
            <div class="mg-today-tick" style="left:${nowPct.toFixed(2)}%" title="Hoje"></div>
        </div>
        <div class="mg-rows">
            <div class="mg-today-bar mg-today-positioned" data-pct="${nowPct.toFixed(4)}" title="Hoje"></div>
            ${rows}
        </div>
        ${noDlRows}
    </div>`;
