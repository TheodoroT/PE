// ============================================================
//  PLANEJAMENTO ESTRATÉGICO — script.js  v4.0
// ============================================================
...
function saveState() { localStorage.setItem('pe_v6', JSON.stringify(state)); }
...
function init() {
    ...
    initOnboarding();
    // Show tooltip for first section
...
function applyConfig() {
    ...
    const n = el('header-username'); if (n) n.textContent = c.userName||'Planejamento Estratégico';
    const ph = el('header-phrase'); if (ph) ph.textContent = c.inspirationalPhrase||'';
...
function setupEventListeners() {
    ...
    if (tl) {
        tl.addEventListener('click', handleTaskClick);
    ...
    document.getElementById('roadmap-content')?.addEventListener('click', e => {
        ...
        if(b) { switchSection('metas'); switchPillar(b.dataset.pillar); }
    });
...
function renderContagemHistory(t) {
    ...
    html+=`<div class="hist-section-label" style="margin-top:16px">Atividade mensal</div>`;
        ...
        html+=`<div class="hist-month">
            ...
            <div class="hist-cal-grid">
                ...
                ${Array.from({length:daysInMonth},(_,i)=>{
                    ...
                    return `<div class="hist-cal-day ${isFuture?'future':intensity}" title="${d}: ${n}">${i+1}</div>`;
                }).join('')}
            </div>
        </div>`;
    });
...
function renderHabitoHistory(t) {
    ...
    // Individual checkins list with delete
            ...
            ${g.tasks.map(t=>{
                ...
                return `<div class="roadmap-item priority-${t.priority}${['Concluída','Encerrada'].includes(t.taskStatus)?' finished':''}">
                    <div class="roadmap-item-top">
                        ...
                        <div class="roadmap-item-right"><span class="roadmap-pct">${pct}%</span><span class="meta-manual-status">${t.taskStatus}</span></div>
                    </div>
...
// ---- SETTINGS ----
function openSettings() { populateSettingsForm(); openSidePanel('settings-overlay'); }
